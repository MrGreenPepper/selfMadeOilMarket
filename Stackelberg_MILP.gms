...
*total cost per unit of oil in dollar per million barrel
...
price(consumer)..                     p(consumer) =e=  (INT(consumer)+SLP(consumer)*d(consumer))*1;
...
of..      mminus_profit_leader =e= sum(consumer, - sum(leader, ql(leader, consumer)*cost(leader, consumer))-sum(follower, qf(follower, consumer)*cost(follower, consumer))- sum(follower,TransportationCap(follower, consumer)*qf_max(follower, consumer) ))
                                    + sum(consumer, baseUtility(consumer)*d(consumer))-sum(consumer,maxConsumption(consumer)*d_max(consumer) );
...
solve stckl using MIP maximising mminus_profit_leader ;
